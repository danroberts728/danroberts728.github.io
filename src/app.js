...
function render() {
    ...
    for (const p of items) {
        ...
        const liveHref = (p.liveUrl || "").trim();

        // treat as "no live site" if it's empty/whitespace, about:blank/#,
        // or just the repo URL itself (some folks put that in homepage)
        const isValidLive =
            liveHref.startsWith("http") &&
            !/github\.com\/[^/]+\/[^/]+\/?$/.test(liveHref) &&
            liveHref !== p.codeUrl;
        ...
        if (isValidLive) {
            ...
            live.remove(); // nuke it so it can't show up
        }
